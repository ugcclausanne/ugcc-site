...
export function Schedule() {
  ...
  const load = async () => {
    ...
    try {
      ...
      setItems(results)
    } catch { setError('Не вдалося завантажити розклад') }
  ...
  const counts = useMemo(() => {
    ...
    const byKind = items.reduce((acc, it) => { acc[it.category||'']=(acc[it.category||'']||0)+1; return acc }, {})
    return { total, byKind }
  }, [items])
...
function emptyLang(lang) { return { language: lang, category: 'liturgy', title: '', date: '', time:'', location:'', details:'', images: [] } }
...
function Editor({ uid, lang, setLang, onClose, onSaved }) {
  ...
  const onChange = (field, value) => { st.setState((s)=>({ ...s, langs:{ ...s.langs, [lang]: { ...s.langs[lang], [field]: value } } })) }
  const onUpload = async (files) => { if (!files?.length) return; const arr=Array.from(files); setPending((p)=>[...p,...arr]); st.setState((s)=>({ ...s, langs:{ ...s.langs, [lang]: { ...s.langs[lang], images:[...(s.langs[lang].images||[]), ...arr.map(f=>f.name)] } } })) }

  const translateMissing = async () => {
    const langs=['uk','en','fr']; const src=st.langs.uk||st.langs[langs.find(l=>st.langs[l]?.title)]; if(!src) return
    for(const l of langs){ if(st.langs[l]?.title) continue; const t={...st.langs[l], language:l}; t.title=await translateLibre(LIBRE,src.title,l,src.language); t.details=await translateLibre(LIBRE,src.details,l,src.language); if(src.location) t.location=await translateLibre(LIBRE,src.location,l,src.language); st.setState((s)=>({ ...s, langs:{ ...s.langs, [l]: t } })); await new Promise(r=>setTimeout(r,150)) }
  }
  ...
  const save = async () => {
    ...
    try {
      ...
      const r = await getRepo(owner, repo, token); const base=r.default_branch||'main'; const sha=await getBranchSha(owner, repo, base, token); const branch=`content/schedule-${uid}-${Date.now()}`; await createBranch(owner, repo, branch, sha, token)
      setStatus('Завантажуємо зображення…')
      for (const f of pending){ const buf=await f.arrayBuffer(); const b64=btoa(String.fromCharCode(...new Uint8Array(buf))); const p=`${st.base}/images/${f.name}`; await putFile(owner, repo, p, b64, `upload image ${f.name}`, token, undefined, branch) }
      if(removed.length){ try{ const ims=await ls(owner,repo,`${st.base}/images`,token); const map=new Map(ims.map((im)=>[im.name,im.sha])); for(const name of removed){ const sha=map.get(name); if(sha) await deleteFile(owner,repo,`${st.base}/images/${name}`,`delete image ${name}`,token,sha,branch) } }catch{} }
      setStatus('Зберігаємо JSON…')
      const arr=['uk','en','fr'].map((k)=>{ const entry={...st.langs[k]}; if(removed.length&&Array.isArray(entry.images)) entry.images=entry.images.filter((n)=>!removed.includes(n)); return entry }); const json=JSON.stringify(arr,null,2); const b64=btoa(unescape(encodeURIComponent(json))); await putFile(owner,repo,st.path,b64,`save schedule ${uid}`,token,undefined,branch)
      setStatus('Створюємо PR…'); const pr=await createPR(owner,repo,`Content: schedule ${uid}`,branch,base,'Edit via admin',token); await enableAutoMerge(owner,repo,pr.node_id,token).catch(()=>{}); setStatus(`PR створено: #${pr.number}. Авто-мердж після перевірок.`); window.dispatchEvent(new CustomEvent('admin:notice',{detail:'Зміни збережено. Публікація відбудеться автоматично після перевірок.'})); onSaved?.()
